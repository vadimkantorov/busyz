//usage:				 "busyz zip [-r] [[-x EXCLUDED_PATH] ...] OUTPUT_NAME.zip INPUT_PATH [...]"
...
int proc_entry(const char *file_path_src, const struct stat *info, const int typeflag, struct FTW *pathinfo)
{
    ...
    if(add)
    {
        ...
        if(typeflag == FTW_D)
        {
            ...
            mz_zip_writer_add_mem(ptr_zip, file_path_dst[0] == '/' ? file_path_dst + 1 : file_path_dst, NULL, 0, 0); // | MZ_ZIP_FLAG_WRITE_HEADER_SET_SIZE
        }
        else if(typeflag == FTW_F)
            mz_zip_writer_add_file(ptr_zip, file_path_dst[0] == '/' ? file_path_dst + 1 : file_path_dst, file_path_src, "", 0, MZ_BEST_COMPRESSION); // | MZ_ZIP_FLAG_WRITE_HEADER_SET_SIZE
    }
